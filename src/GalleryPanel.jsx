...
export default function GalleryPanel({ allPhotos, allPhotoCaptions, polaroidMode, onTogglePolaroid, onSelectPhoto, onClose, isMobile }) {
  const P = getP();
  const desktopStyle = { position: "absolute", top: 72, left: 22, zIndex: 22, width: 290, maxHeight: "calc(100vh - 200px)", borderRadius: 18, animation: "fadeIn .4s ease" };
  const mobileStyle = { position: "fixed", bottom: 0, left: 0, right: 0, top: "auto", zIndex: 22, width: "100%", height: "60vh", borderRadius: "16px 16px 0 0", animation: "fadeIn .3s ease" };
  return (
    <div style={{ ...(isMobile ? mobileStyle : desktopStyle), background: P.card, backdropFilter: "blur(28px)", boxShadow: "0 1px 3px rgba(61,53,82,.04), 0 8px 24px rgba(61,53,82,.06), 0 20px 48px rgba(61,53,82,.08)", border: `1px solid ${P.rose}08`, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      {isMobile && <div style={{ display: "flex", justifyContent: "center", padding: "8px 0 0" }}><div style={{ width: 36, height: 4, borderRadius: 2, background: P.textFaint + "40" }} /></div>}
      <div style={{ padding: "12px 14px 8px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, borderBottom: `1px solid ${P.rose}08` }}>
        <div>
          ...
          <div style={{ fontSize: 10, color: P.textFaint, letterSpacing: ".1em", marginTop: 1 }}>{allPhotos.length} memories</div>
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", padding: "0 10px 4px" }}>
        <button onClick={onTogglePolaroid} style={{ background: polaroidMode ? `${P.goldWarm}18` : "none", border: `1px solid ${polaroidMode ? P.goldWarm + "30" : P.textFaint + "20"}`, borderRadius: 6, padding: "2px 8px", fontSize: 10, cursor: "pointer", fontFamily: "inherit", color: polaroidMode ? P.goldWarm : P.textFaint }}>{polaroidMode ? "📸 Polaroid" : "▦ Grid"}</button>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: 8 }}>
        <div style={{ display: "grid", gridTemplateColumns: polaroidMode ? "1fr 1fr" : "1fr 1fr 1fr", gap: polaroidMode ? 14 : 4, padding: polaroidMode ? "4px 6px" : 0 }}>
          {allPhotos.map((ph, i) => (
            <button key={i} onClick={() => onSelectPhoto(ph)}
              ...
              <div style={polaroidMode ? { fontSize: 10, color: "#666", textAlign: "center", padding: "4px 3px 0", letterSpacing: ".03em", fontFamily: allPhotoCaptions[ph.url] ? "'Palatino Linotype','Book Antiqua',Palatino,Georgia,serif" : "inherit", fontStyle: allPhotoCaptions[ph.url] ? "italic" : "normal", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } : { position: "absolute", bottom: 0, left: 0, right: 0, padding: "10px 3px 2px", background: "linear-gradient(transparent, rgba(0,0,0,.5))", fontSize: 10, color: "#fff", textAlign: "center", letterSpacing: ".05em" }}>{polaroidMode && allPhotoCaptions[ph.url] ? allPhotoCaptions[ph.url] : ph.city}</div>
            </button>
          ))}
        </div>
