import { DreamAddForm, DREAM_CATEGORIES } from "./EntryForms.jsx";

const todayStr = () => new Date().toISOString().slice(0, 10);

export default function DreamPanel({ dreams, visitedCount, palette, isMyWorld, isPartnerWorld, isViewer, worldType, onClose, onAddDream, onRemoveDream, onConvertToEntry }) {
  const P = palette;
        ...
        {dreams.map((dream) => {
          ...
          <div key={dream.id} style={{ padding: "10px 12px", background: `${P.gold}08`, borderRadius: 10, marginBottom: 6, borderLeft: `3px solid ${P.goldWarm}40` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {!isViewer && (<>
                <button onClick={() => onConvertToEntry(dream)} style={{ background: P.rose, color: "#fff", border: "none", borderRadius: 5, padding: "3px 8px", fontSize: 8, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>✓ Visited!</button>
