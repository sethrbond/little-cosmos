...
export default function TimelineSlider({ entries, sorted, sliderDate, onSliderChange, milestones, chapters, palette, onSelectEntry, effectiveStartDate, isPartnerWorld, isMyWorld, isAnimating, areTogether, pos, stepDay, jumpNext, navStyle, TYPES, DEFAULT_TYPE, selected, flyTo }) {
  const P = palette;
        ...
        <div style={{ minWidth: 150, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 10, color: isMyWorld ? P.textMid : (isPartnerWorld && areTogether ? P.heart : P.textFaint), letterSpacing: ".1em", marginTop: 1 }}>
            {isMyWorld
              ...
              : (entries.length > 0 ? `📍 ${entries[0]?.city || ""}` : "Add entries to begin")
            }
          </div>
        </div>
        <button onClick={() => stepDay(1)} disabled={isAnimating} style={navStyle()}>▸</button>
