...
import { _symbolCache, _meteorV1, _meteorV2, createPool, ll2v, lerp, clamp, LAND, COAST_DATA, RAD } from "./globeUtils.js";
...
/**
 * useGlobeScene — custom hook that owns the entire Three.js scene lifecycle.
 *
 * @param {React.RefObject} mountRef  — ref to the DOM element the renderer attaches to
 * @param {object} deps               — all external refs / values the scene reads
 * @returns {{ sceneReady: boolean, rendRef, scnRef, camRef, globeRef, heartRef,
...
export function useGlobeScene(mountRef, deps) {
  const {
    ...
    isPartnerWorld,
    isSharedWorld,
    // Refs the animation loop reads from (owned by OurWorld)
    selectedRef,
    searchMatchIdsRef,
    mkRef,
    routesRef,
    ...
    dismissTimers,
    pulseRingsRef,
    // Refs that cleanup nulls (owned by OurWorld)
    rtRef,
    loveThreadRef,
    constellationRef,
    tripRouteRef,
  ...
  // Scene-internal refs — created and returned by this hook
  ...
  const pulseRingPoolRef = useRef(null);
  const burstParticlePoolRef = useRef(null);
