...
function getLetterCopy(worldType, isMyWorld, config) {
  if (isMyWorld || worldType === "personal") {
    return {
      title: "Dear Future Me",
      newTitle: "Note to Future Me",
      editTitle: "Edit Note",
      placeholder: "Dear future me...",
      hint: "A note to your future self, hidden on the globe",
      discoverHint: "you'll rediscover it later!",
      buttonLabel: "Note",
      markerIcon: "✦",
      sendEmoji: "✉️",
      toastSent: "Note saved! ✉️",
      toastHidden: "Note hidden on the globe ✦",
      ariaView: "Personal note",
      ariaEdit: "Edit personal note",
    };
  }
  if (worldType === "friends") {
    return {
      title: "Note to the Crew",
      newTitle: "Note to the Crew",
      editTitle: "Edit Note",
      placeholder: "Hey everyone...",
      hint: "Leave a message for the group, hidden on the globe",
      discoverHint: "the crew will discover it!",
      buttonLabel: "Note",
      markerIcon: "✧",
      sendEmoji: "✉️",
      toastSent: "Note sent! ✉️",
      toastHidden: "Note hidden on the globe ✧",
      ariaView: "Group note",
      ariaEdit: "Edit group note",
    };
  }
  if (worldType === "family") {
    return {
      title: "Family Note",
      newTitle: "Family Note",
      editTitle: "Edit Note",
      placeholder: `Dear ${config.worldName || "family"}...`,
      hint: "Write a note for the family, hidden on the globe",
      discoverHint: "the family will discover it!",
      buttonLabel: "Note",
      markerIcon: "♥",
      sendEmoji: "💕",
      toastSent: "Note sent! 💕",
      toastHidden: "Note hidden on the globe ♥",
      ariaView: "Family note",
      ariaEdit: "Edit family note",
    };
  }
  // Partner (default)
  return {
    title: "Love Letter",
    newTitle: "Love Letter",
    editTitle: "Edit Love Letter",
    placeholder: `Dear ${config.partnerName || "Partner"}...`,
    hint: `Hidden as an easter egg ❀ on the globe — ${config.partnerName || "your partner"} will discover it!`,
    discoverHint: null, // not used — hint is already complete
    buttonLabel: "Love Letter",
    markerIcon: "❀",
    sendEmoji: "💌",
    toastSent: "Letter sent! 💌",
    toastHidden: "Letter hidden on the globe ❀",
    ariaView: "Love letter",
    ariaEdit: "Edit love letter",
  };
}
    ...
    <div role="dialog" aria-modal="true" aria-label={copy.ariaView} onClick={onClose} style={{ position: "absolute", inset: 0, zIndex: 50, background: `linear-gradient(135deg, rgba(22,16,40,.84), rgba(30,24,48,.90))`, backdropFilter: "blur(30px)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", animation: "fadeIn .8s ease" }}>
      <div style={{ maxWidth: 460, padding: 36, textAlign: "center" }} onClick={e => e.stopPropagation()}>
        ...
        {isMyLetter && <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 14 }}>
          ...
          {letter.draft && <button onClick={() => onSend(letter)} style={{ background: `${P.rose}20`, border: `1px solid ${P.rose}40`, borderRadius: 5, padding: "4px 12px", fontSize: 9, color: P.rose, cursor: "pointer", fontFamily: "inherit" }}>Send {copy.sendEmoji}</button>}
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={copy.ariaEdit} onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 55, background: "rgba(22,16,40,.82)", backdropFilter: "blur(20px)", display: "flex", alignItems: "center", justifyContent: "center", animation: "fadeIn .2s ease" }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 420, maxWidth: "92vw", padding: 28, background: P.card, borderRadius: 16, boxShadow: "0 14px 48px rgba(61,53,82,.1)" }}>
        ...
        <textarea value={letterDraft} onChange={e => setLetterDraft(e.target.value)} rows={8} placeholder={copy.placeholder} style={{ ...inputStyle(), resize: "vertical", lineHeight: 1.8 }} />
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <button onClick={() => { onSave(letterEditId, buildObj(false)); }} disabled={!letterDraft.trim()} style={{ flex: 1, padding: "10px", background: letterDraft.trim() ? `linear-gradient(135deg, ${P.rose}, ${P.sky})` : `${P.textFaint}60`, color: "#fff", border: "none", borderRadius: 12, cursor: letterDraft.trim() ? "pointer" : "default", fontSize: 11, fontFamily: "inherit", letterSpacing: ".04em", boxShadow: letterDraft.trim() ? `0 2px 8px ${P.rose}30` : "none", transition: "all .25s" }}>
            {letterEditId ? "Update & Send" : "Hide on Globe"} {copy.sendEmoji}
          </button>
...
export { getLetterCopy };

export default function LoveLetterOverlay({ showLetterId, editLetter, letters, config, userId, letterEditId, initialDraft, initialCity, initialLat, initialLng, onCloseLetter, onCloseEdit, onEditLetter, onSendLetter, onRemoveLetter, onSaveLetter, onSaveDraft, worldType, isMyWorld }) {
  const copy = getLetterCopy(worldType, isMyWorld, config);
  return (
    <>
      {showLetterId && (() => {
        const letter = (letters || []).find(l => l.id === showLetterId);
        if (!letter) return null;
        return <LetterViewModal letter={letter} userId={userId} config={config} onClose={onCloseLetter} onEdit={onEditLetter} onSend={onSendLetter} onRemove={onRemoveLetter} copy={copy} />;
      })()}
      {editLetter && (
        <LetterEditModal
