...
import {
  ...
  getSharedWorldConfig, WORLD_THEMES,
...
export default function SettingsPanel({
  config, setConfig, isMyWorld, isPartnerWorld, isSharedWorld, worldType,
  worldId, userId, worldRole, worldName,
  handwrittenMode, setHandwrittenMode,
  ambientRef, ambientPlaying,
  onClose, showToast, flushConfigSave, exportData, importData,
  onSwitchWorld, setConfirmModal,
  onboardKey, setShowOnboarding, setOnboardStep,
}) {
  const P = getP();
  const trapRef = useFocusTrap(true);

  // --- color picker state (moved from OurWorld) ---
  ...
  // Load members + letters on mount
  const [loaded, setLoaded] = useState(false);
  if (!loaded) {
    setLoaded(true);
    if (isSharedWorld && worldId) {
      getWorldMembers(worldId).then(setWorldMembers).catch(() => {});
    }
    if (userId) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Settings" onClick={handleClose} style={{ position: "absolute", inset: 0, zIndex: 45, background: `linear-gradient(135deg, rgba(22,16,40,.82), rgba(30,24,48,.88))`, backdropFilter: "blur(24px)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", animation: "fadeIn .3s ease" }}>
      <div ref={trapRef} onClick={e => e.stopPropagation()} style={{ width: 400, maxWidth: "92vw", maxHeight: "85vh", overflowY: "auto", padding: 30, background: P.card, borderRadius: 22, boxShadow: "0 1px 3px rgba(61,53,82,.04), 0 8px 24px rgba(61,53,82,.06), 0 24px 64px rgba(61,53,82,.1)", cursor: "default" }}>
        ...
        <div style={{ padding: 14, borderRadius: 10, background: "rgba(200,80,80,.04)", border: "1px solid rgba(200,100,100,.15)" }}>
          ...
          <input
            ...
            style={{ ...inputStyle(), marginBottom: 8, borderColor: deleteConfirm === "DELETE" ? "rgba(200,80,80,.5)" : undefined }}
          />
          <button
            ...
            style={{
              ...
              background: deleteConfirm === "DELETE" && !deleting ? "rgba(200,80,80,.9)" : "rgba(200,80,80,.2)",
              border: "1px solid rgba(200,100,100,.3)", borderRadius: 8,
              cursor: deleteConfirm === "DELETE" && !deleting ? "pointer" : "not-allowed",
              fontSize: 10, fontFamily: "inherit", color: deleteConfirm === "DELETE" ? "#fff" : "#c97777",
              fontWeight: 600, letterSpacing: ".04em", transition: "all .2s",
