...
export default function PhotoLightbox({ photos, lightboxIdx, setLightboxIdx, onClose, isViewer, dispatch, entryId, photoCaptions, P, city, country }) {
  const idx = ((lightboxIdx % photos.length) + photos.length) % photos.length;
  const caption = (photoCaptions || {})[photos[idx]];
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Photo lightbox" style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.92)", backdropFilter: "blur(24px)", display: "flex", alignItems: "center", justifyContent: "center", animation: "fadeIn .25s ease" }}
      ...
      <div onClick={e => e.stopPropagation()} style={{ position: "absolute", bottom: photos.length > 1 ? 72 : 46, left: 0, right: 0, textAlign: "center", zIndex: 210, animation: "lbFadeOpacity .5s ease" }}>
        {!isViewer ? (
          <input
            ...
            onChange={e => {
              const captions = { ...(photoCaptions || {}), [photos[idx]]: e.target.value };
              dispatch({ type: "UPDATE", id: entryId, data: { photoCaptions: captions }, _skipSave: true });
            }}
            onBlur={e => {
              const captions = { ...(photoCaptions || {}), [photos[idx]]: e.target.value };
              dispatch({ type: "UPDATE", id: entryId, data: { photoCaptions: captions } });
            }}
      ...
      {/* City label */}
      <div style={{ position: "absolute", bottom: photos.length > 1 ? 50 : 24, left: 0, right: 0, textAlign: "center", color: "rgba(255,255,255,0.4)", fontSize: 11, letterSpacing: "1px", zIndex: 210 }}>{city}{country ? `, ${country}` : ""}</div>
    </div>
  );
}
