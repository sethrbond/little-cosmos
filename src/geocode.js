...
export function geocodeSearch(query, callback) {
  ...
  debounceTimer = setTimeout(async () => {
    ...
    try {
      ...
      const seen = new Set()
      const unique = results.filter(r => { const key = `${r[0]}|${r[1]}`; if (seen.has(key)) return false; seen.add(key); return true })
      callback(unique.slice(0, 8))
    } catch (err) { console.error('[geocode] error:', err); callback([]) }
  }, DEBOUNCE_MS)
}
