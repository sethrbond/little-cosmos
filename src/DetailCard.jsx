...
/**
 * DetailCard — renders the selected entry detail overlay:
 *   photo carousel, tabs (overview/highlights/places/photos),
 *   reactions, comments, navigation arrows, and fullscreen lightbox.
 *
 * Photo-related UI state lives here (photoIdx, lightboxOpen, cardTab, etc.)
 * to keep OurWorld lean. Mutations flow up via callback props.
 */
export default function DetailCard({
  ...
  onTripCard,
  onConfirmDelete,
  // reactions & comments (shared world)
  worldReactions,
  setWorldReactions,
  entryComments,
  setEntryComments,
  // upload state (managed by parent for the hidden file input)
  uploadLockRef,
  setUploading,
  setUploadProgress,
  // mobile
  ...
  // slider
  setSliderDate,
  // togetherIndex callback (partner worlds)
  togetherIndex,
}) {
  // ---- LOCAL STATE (photo/card UI) ----
  ...
  const [shareWorlds, setShareWorlds] = useState(null);
  const [showLinkPicker, setShowLinkPicker] = useState(false);
  const [linkedEntryId, setLinkedEntryId] = useState(null);
  ...
  }, [cur?.id, cur?.photos?.length]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Photo drag-drop upload handler (shared between photo area and empty drop zone)
  const handleDragDropUpload = useCallback((e) => {
  ...
  }, [cur?.id, db, dispatch, showToast, uploadLockRef, setUploading, setUploadProgress]);
    ...
    <div style={isMobile
      ...
      {(cur.photos || []).length > 0 && !cardGallery && (
        <div
          ...
          onDrop={!isViewer ? handleDragDropUpload : undefined}
      ...
      <div style={{ padding: "14px 18px 18px", overflowY: "auto", flex: 1 }}>
        <div style={{ float: "right", display: "flex", gap: 2, marginTop: -4, position: "relative" }}>
          ...
          <button aria-label="Close detail card" onClick={onClose} style={{ background: "none", border: "none", fontSize: 16, color: P.textFaint, cursor: "pointer", marginLeft: 2, minWidth: 44, minHeight: 44, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
        </div>
        ...
        {sorted.length > 1 && (() => {
          ...
          return (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, paddingTop: 8, borderTop: `1px solid ${P.rose}10` }}>
              <button disabled={!prev} onClick={() => { if (prev) { onSelectEntry(prev); if (setSliderDate) setSliderDate(prev.dateStart); flyTo(prev.lat, prev.lng); } }}
              ...
              <button disabled={!next} onClick={() => { if (next) { onSelectEntry(next); if (setSliderDate) setSliderDate(next.dateStart); flyTo(next.lat, next.lng); } }}
