...
export default function SearchPanel({ entries, types, defaultType, palette, isMobile, onSelectEntry, onClose, searchMatchIdsRef, isSharedWorld, memberNameMap }) {
  ...
  useEffect(() => {
    if (searchMatchIdsRef) {
      searchMatchIdsRef.current = new Set(searchResults.map(e => e.id));
    }
  }, [searchResults, searchMatchIdsRef]);

  // Clear match IDs on unmount
  useEffect(() => {
    return () => {
      if (searchMatchIdsRef) searchMatchIdsRef.current = new Set();
    };
  }, [searchMatchIdsRef]);
        ...
        <input autoFocus value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setSearchHl(-1); }}
          onKeyDown={e => {
            if (e.key === "ArrowDown") { e.preventDefault(); setSearchHl(h => Math.min(h + 1, searchResults.length - 1)); }
              ...
              e.preventDefault();
              handleSelect(searchResults[searchHl]);
            }
      ...
      {hasSearchFilters && (
        <div style={{ marginTop: 4, background: P.card, backdropFilter: "blur(16px)", borderRadius: 10, maxHeight: 300, overflowY: "auto", boxShadow: "0 8px 28px rgba(61,53,82,.12)", border: `1px solid ${P.rose}10`, animation: "fadeIn .2s ease" }}>
          ...
          {searchResults.slice(0, 50).map((e, ri) => {
            ...
            return (
              <button key={e.id} onClick={() => handleSelect(e)} style={{ display: "flex", width: "100%", alignItems: "center", gap: 8, padding: "9px 14px", border: "none", borderBottom: `1px solid ${P.parchment}`, background: isHl ? P.blush : "transparent", cursor: "pointer", fontFamily: "inherit", textAlign: "left" }}
